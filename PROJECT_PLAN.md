...
*   `[completed]` **Task 2.1 (Active Sensing): Integrate & Model Complex Audio Sources**
    ...
    *   `[completed]` 2.1.4: Multi-pose aggregation — K (driver, mic-pair) poses per room, exploiting laptop movement instead of adding channels (fits the laptop-only hardware constraint). **Best Phase 2 recipe: joint-trained encoder run per pose + Bayes product fusion — held-out IoU 0.0924 at K=8, 2.4× the single-pose baseline.**
        ...
        - `[completed]` 2.1.4c: `JointPoseCNN` — shared pose encoder + mean-pooled latents, param-matched to the single-pose model. Trained on 10k rooms × 4 poses: beats single-pose training at every K (held-out 0.047 vs 0.037 at K=4; 0.056 vs 0.038 as a single-pose predictor) but mean-pool fusion degrades with K (1/K latent-variance shrinkage); explicit Bayes fusion over its per-pose outputs is the campaign best (0.0924 @ K=8). See `tests/reports/joint_pose_2026_07_11.md`.
...
*   `[completed]` **Task 2.3: Sensing-quality improvements (a–e)**
    ...
    *   `[completed]` Training + evaluation: **held-out IoU 0.100–0.101 at the calibrated operating point** (vs v1 recipe 0.094). Attribution via the joint_v2 ablation: (e) ≈ +6 % and equalises architectures, (b+c) ≈ +5 %, (a+d) ≈ 0 at the ceiling — two different architectures converge on ≈ 0.10, indicating the acquisition's information ceiling. See `tests/reports/sensing_v2_2026_07_15.md`.
