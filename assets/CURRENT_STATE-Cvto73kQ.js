var e=`# Project Status as of 2026-09-24
...
|---|---|---|
| 3 Debug audit | in progress | 18 engine, 12 backend and 10 learning findings fixed or retired (\`tests/reports/debug_audit_2026_09_24.md\`). The retrained leak-free skip_v2 still sits on the no-audio baseline (IoU 0.104 vs 0.101 at K = 4, z = 1.1; debug audit §3). Datasets and checkpoints are in the GitHub Release \`data-v2.0\` (3.5.3). **Open:** GPU run (3.2.3, needs hardware). |
...
| 8 Closed loop | done | \`#/loop\`: sense → twin → ACC → measure, through a moving listener, a moving obstacle and a new partition. The monitor-guarded loop keeps 16–41 dB; the loop period is about 5 s. A learned room estimate (8.7) raises IoU from 0.18 to 0.79 and contrast by 9.5 dB (z = 12.6), closing 69 % of the gap to the oracle on rooms like its training set, but not on unseen shapes (\`tests/reports/loop_sensing_2026_09_25.md\`). |
...
| 10 Showcase | mostly done | Pages deploy, WebGPU engine (parity 1e-6), in-browser CNN (parity 1e-5), 15-scene gallery, 6 explainers, docs site, GIF renderer, benchmark, 4 write-ups. Data release \`data-v2.0\` (10.10). **Open:** the real-hardware write-up (needs measurements). |
...
`;export{e as default};
//# sourceMappingURL=CURRENT_STATE-Cvto73kQ.js.map