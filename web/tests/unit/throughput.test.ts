...
// Regression guard, not a benchmark: the bounds catch order-of-magnitude slowdowns.
it('general kernel is within 5x of the fast path', () => {
  ...
  expect(pml).toBeGreaterThan(fast / 5);
  expect(cpml).toBeGreaterThan(fast / 8); // CPML adds two memory variables per axis in the layer
